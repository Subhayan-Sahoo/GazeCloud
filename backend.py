...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "https://gazecloud-1.onrender.com"
    ],
...
def clean_point(x, y, width=1920, height=1080):
...
def generate_heatmap(participant_id: str, width: int, height: int, document_id: str | None = None, device: str | None = None, trial_index : int | None=None, layout_image: str | None = None):
    ...
    # 🔥 Optimized Heatmap (vectorized + downsampled)
    scale = 4  # reduce resolution (tune: 2–6)
    ...
    # Filter valid points
    coords = coords[
        (coords[:, 0] >= 0) & (coords[:, 0] < small_w) &
        (coords[:, 1] >= 0) & (coords[:, 1] < small_h)
    ]

    # Fast accumulation (vectorized)
    np.add.at(heatmap, (coords[:, 1], coords[:, 0]), 1)

    # Smooth with single gaussian
    heatmap = gaussian_filter(heatmap, sigma=SIGMA / scale)
    if np.max(heatmap) == 0:
...
@app.post("/log-batch")
def log_batch(payload: BatchGazeLog, request: Request):
    rate_limit(request)
    ...
    for item in payload.data:
        pt = clean_point(item.get("x"), item.get("y"))
        if not pt:
            continue  # skip invalid points
        x,y = pt
        cur.execute("""
            INSERT INTO gaze_logs 
            (participant_id, session_id, x, y, document_id, trial_index, device, sample_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, 'gaze')
        """, (
...
def generate_heatmap_endpoint(req: HeatmapRequest, request: Request):
    rate_limit(request)
    try:
        _, filename = generate_heatmap(req.participant_id, req.width, req.height, req.document_id, req.device, req.trial_index, req.layout_image)
