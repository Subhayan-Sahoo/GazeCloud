const API_BASE = "http://gazecloud.onrender.com";
...
function buildCalibrationGrid() {
  ...
  positions.forEach(([rx, ry], idx) => {
    ...
    el.style.cssText = "position:absolute;width:22px;height:22px;background:#2980b9;border-radius:50%;transform:translate(-50%,-50%);cursor:pointer;color:#fff;display:flex;align-items:center;justify-content:center;font-size:11px;";
...
function activateCalibrationPoint(index) {
  calibrationGrid.querySelectorAll("div").forEach((p) => {
  ...
  const point = calibrationGrid.children[index];
...
async function finishExperiment() {
  ...
  GazeRecorderAPI.Stop();
  const fullData = GazeRecorderAPI.GetData();
