// ⚠️ IMPORTANT: This must be your BACKEND service URL on Render, not the frontend URL.
// Frontend: https://gazecloud-1.onrender.com  (static site)
// Backend:  https://gazecloud.onrender.com    (FastAPI)
// If these are swapped, all fetch() calls will CORS-fail.
...
const BATCH_SIZE = 50;
const FLUSH_INTERVAL = 5000; // 3 sec
let uploadQueue = [];
...
GazeCloudAPI.OnResult = function (gd) {
  ...
  // ── 1. HEAD POSE FILTER ────────────────────────────────────────────────────
  if (
    gd.HeadZ < 5 || gd.HeadZ > 150 ||
    Math.abs(gd.HeadYaw) > 45 ||
    Math.abs(gd.HeadPitch) > 45
  ) {
    setStatus("Keep head centred");
    return;
  }

  console.log("STATE:", gd.state, "Stable:", stableFrames, "Ready:", isTrackingReady);
  ...
  if (gd.state === 0) {
    ...
    if (stableFrames > 5) {
      ...
      $("start-custom-calibration").disabled = true;
    }
  ...
  if (gazeMode === "reading") {
    ...
    console.log("UPLOAD QUEUE:", uploadQueue.length);
  }
};
  ...
  plain: [
    {
      ...
      lines: [
        ...
        "It offers much higher resolution than earlier instruments and can detect very faint X-ray sources across distant regions of space.",
        "Chandra has helped scientists study interactions between exoplanets and their host stars,",
        "examine the environments around possible planet-bearing systems, and even report evidence of a possible exoplanet beyond the Milky Way galaxy."
      ],
    ...
    },
    {
      ...
      lines: [
        "Physicists at CERN recently reported a major improvement in producing antihydrogen, the antimatter version of hydrogen.",
        "Using a new technique, researchers generated more than fifteen thousand antihydrogen atoms within only a few hours,greatly increasing experimental efficiency.",
        "Antimatter is scientifically important because comparing it with ordinary matter may reveal whether the",
        "laws of physics treat both forms in exactly the same way under controlled experimental conditions.."
      ],
    ...
    },
    {
      ...
      lines: [
        "Neuroscience researchers recently revealed structural mechanisms underlying memory formation across a wide network of neurons in the mouse brain.",
        "Their work highlights the flexible nature of how memories are formed, showing learning-related changes at both cellular and subcellular levels with unusually high resolution. ",
        "Understanding this biological flexibility may help scientists explain why memory and ",
        "learning processes sometimes fail, and how such disruptions may contribute to brain-related disorders."
      ],
    ...
    },
    {
      ...
      lines: [
        "Medical researchers are developing new ways to repair vision by replacing damaged retinal cells in the eye. In a recent study,",
        ", scientists transplanted lab-grown retinal pigment epithelial cells into patients with severe macular degeneration. ",
        "The treatment was intended to support light-sensitive tissue needed for central vision and to slow further deterioration. Early findings suggest that cell-based therapy may become a ",
        "promising option for degenerative eye disease when conventional treatments cannot restore damaged tissue."
      ],
...
async function completeTrial() {
  ...
  //  await fetch(`${API_BASE}/log-fixations`, {
  //    method: "POST",
  //    headers: {
  ...
  if (dataBuffer.length > 0) { uploadQueue.push(...dataBuffer); dataBuffer = []; }
  while(uploadQueue.length>0 ){
    await flushData(true);
    await new Promise(r => setTimeout(r,500));
  }

  // Small wait to let any pending saveToDB() network calls commit before heatmap queries the DB
...
function _advanceTrial() {
  ...
  loadTrial(currentTrialIndex); // loadTrial uses currentTrialIndex directly, already incremented
}
...
async function captureLayoutImage() {
  // html2canvas will throw on GazeCloud's cross-origin canvas (CalCanvasId).
  // We ignore tainted canvases by passing useCORS:false and ignoreElements.
  try {
    ...
    console.warn("captureLayoutImage failed:", e);
    return null;
  }
}
...
async function generateHeatmapForCurrentTrial() {
  try {
    ...
    // Capture layout image - may return null if canvas is tainted, that's OK
    if (!window.cachedLayoutImage) {
      window.cachedLayoutImage = await captureLayoutImage();
    }
    layoutImage = window.cachedLayoutImage; // may be null — backend handles it
    ...
    const response = await fetch(`${API_BASE}/generate-heatmap`, {
      ...
      body: JSON.stringify({
        ...
        device: currentDevice,
        trial_index: currentTrialIndex + 1,
        layout_image: layoutImage  // null = no background overlay, still works
      })
    });
    ...
    hideLoading();
    console.warn("Heatmap skipped:", e.message);
    $("trial-status").textContent = `Saved ${currentStimulus.document_id}. Moving to next…`;
    $("trial-status").style.color = "#495a6a";
    pendingNextTrialIndex = currentTrialIndex+1;
    closeHeatmap();
  }
}
