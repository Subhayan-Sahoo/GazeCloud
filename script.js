...
function startCalibration() {
  if (!isTrackingReady) {
    alert("Please wait for tracking to stabilise first (status shows 'Tracking stable').");
    return;
  }
  isCalibrating = true;
  calibPairs = [];
...
function loadTrial(index) {
  ...
  window.cachedLayoutImage = null; // invalidate layout cache for new passage

  readingStartTime = performance.now();
  startTrialTimer();

  // FIX 3: just flip mode, GazeCloud keeps running
  gazeMode = "reading";
}
...
function showHeatmap() {
  ...
  // Pause gaze tracking display while modal is open (dot hidden)
...
function closeHeatmap() {
  ...
  // FIX 3 + previous bug: advance to next trial AFTER user closes heatmap
  if (pendingNextTrialIndex !== null) {
