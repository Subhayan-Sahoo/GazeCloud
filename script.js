...
GazeCloudAPI.OnCalibrationComplete = function(){
  ...
  if (hasCustomCalibration) {
    console.log("Skipping 9-point calibration, reusing previous affine");
    ...
    resetTrackingState();   // important after restart
  ...
  setTimeout(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        console.log("Waiting for user to start custom calibration...");
      });
    });
  }, 300);
...
function saveCalibrationState() {
  ...
  localStorage.setItem("gaze_calibration", JSON.stringify({
    affine: affine,
    filters: settings
  }));
}
...
function restartGazeTracking() {
  ...
  setTimeout(() => {
    console.log("Restarting GazeCloud with built-in calibration...");

    gazeMode = "restarting";  // just for tracking
...
function loadCalibrationState() {
  ...
  try {
    ...
    console.log("Loaded saved calibration + filters");
    return true;
...
window.addEventListener("beforeunload", saveCalibrationState);
