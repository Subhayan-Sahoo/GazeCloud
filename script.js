...
GazeCloudAPI.OnResult = function(gd) {
  ...
  // ✅ 1. HEAD POSE FILTER (FIRST)
  if (
    gd.HeadZ < 20 || gd.HeadZ > 80 ||
    Math.abs(gd.HeadYaw) > 20 ||
    Math.abs(gd.HeadPitch) > 20
  ) {
    $("status").textContent = "Keep head centered";
    return;
  }

  // ✅ 2. STABILITY CHECK (CORRECTED)
  if (gd.state === 0) {
    stableFrames++;

    if (stableFrames > 15 && !isTrackingReady) {
      console.log("Tracking stabilized ✅");
      isTrackingReady = true;
      $("status").textContent = "Tracking stable ✅";
    }
  } else {
    stableFrames = 0;
    isTrackingReady = false;
    $("status").textContent = "Adjust face / lighting...";
  }

  // ✅ Always update gaze
  lastGaze = { x: gd.GazeX, y: gd.GazeY };

  // ✅ 3. CALIBRATION MODE (ALWAYS SHOW DOT)
  if (gazeMode === "calibration") {
    const rect = calibrationGrid.getBoundingClientRect();

    const localX = Math.max(0, Math.min(rect.width, gd.GazeX - rect.left));
    const localY = Math.max(0, Math.min(rect.height, gd.GazeY - rect.top));

    gazePointEl.style.left = localX + "px";
    gazePointEl.style.top = localY + "px";
    gazePointEl.style.display = "block";

    // ❗ Only allow calibration recording when stable
    if (!isTrackingReady) return;

    return;
  }

  // ❗ STOP EVERYTHING if not stable
  if (!isTrackingReady) return;

  // ✅ 4. BLINK / LOST FRAME LOGIC
  ...
  if (gazeMode === "reading") {
    ...
    gazePointEl.style.left = filtered.x + "px";
    gazePointEl.style.top = filtered.y + "px";
...
function startCalibration() {
  ...
  if (!isTrackingReady) {
    alert("Please stabilize tracking first (adjust face/lighting)");
    return;
  }
...
function startTrialTimer() {
  ...
  trialTimer = setInterval(() => {
    ...
    if (trialTimeLeft <= 0) {
      ...
      console.log("Auto next -> restarting tracking");
      restartGazeTracking();
      nextDocument();
    }
  }, 1000);
...
window.onload = function(){
  ...
  $("next-document").onclick = async function () { 
    $("status").textContent = "Restarting tracker...";
    console.log("User triggered next document -> restart tracking");
    restartGazeTracking();
    setTimeout(async() => {
      await nextDocument();
    }, 1000);
